...
class PlayState extends MusicBeatState
{
	...
	// Lua shit
	public static var instance:funkin.game.PlayState;
	...
	function startSong():Void
	{
		...
		FlxG.sound.playMusic(Paths.inst(funkin.game.PlayState.SONG.song), 1, false);
		FlxG.sound.music.pitch = playbackRate;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		FlxG.sound.list.add(new FlxSound().loadEmbedded(Paths.inst(funkin.game.PlayState.SONG.song)));
						...
						if(daNote.copyY)
						{
							...
							if(strumScroll && daNote.isSustainNote)
							{
								if (daNote.animation.curAnim.name.endsWith('end')) {
									...
									if(funkin.game.PlayState.isPixelStage) {
										daNote.y += 8 + (6 - daNote.originalHeightForCalcs) * funkin.game.PlayState.daPixelZoom;
									} else {
										daNote.y -= 19;
									}
								}
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
						funkin.menus.StoryMenuState.weekCompleted.set(WeekData.weeksList[storyWeek], true);
						...
						FlxG.save.data.weekCompleted = funkin.menus.StoryMenuState.weekCompleted;
						FlxG.save.flush();
					}
				...
				else
				{
					...
					trace(Paths.formatToSongPath(funkin.game.PlayState.storyPlaylist[0]) + difficulty);
					...
					funkin.game.PlayState.SONG = Song.loadFromJson(funkin.game.PlayState.storyPlaylist[0] + difficulty, funkin.game.PlayState.storyPlaylist[0]);
					FlxG.sound.music.stop();
